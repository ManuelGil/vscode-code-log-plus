...
const discoverFilesLocally = async (
  ...
  const globStream: AsyncIterable<string | Buffer> & { destroy?: () => void } =
    FastGlob.stream(normalizedIncludeGlobs, {
  ...
  try {
    for await (const matchedFilePathRaw of globStream) {
      if (!matchedFilePathRaw) {
        continue;
      }

      const matchedFilePath =
        typeof matchedFilePathRaw === 'string'
          ? matchedFilePathRaw
          : matchedFilePathRaw.toString();

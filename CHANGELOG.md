...
- Log operations now support selection, function, and file scopes.
- Refactor extension initialization logic into `ExtensionRuntime` class for improved maintainability
- Centralize command configuration with `CommandIds` enum for better command management
- Extract and organize context state management with `GlobalStateKeys` enum
- Introduce view configuration with `ViewIds` enum for consistent view management
- Improve TypeScript configuration for better Node.js compatibility
- Update VSCode engine requirement to ^1.102.0 for latest platform features
- Reorganize extension architecture for enhanced code organization and scalability
...
[Unreleased]: https://github.com/ManuelGil/vscode-code-log-plus/compare/v1.11.0...HEAD
